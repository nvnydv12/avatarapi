...
API_KEY = "PANKAJ"
INFO_API_URL = "https://your-info-api/info"
...
def load_font(candidates, size):
    ...
    if font_path:
        try:
            ...
            pass
    # Last fallback: scaled PIL bitmap font, so text does not become tiny.
...
def text_width(draw, text, render_font, stroke_width=0):
    ...
    for part in re.split(r'()', text):
        ...
        if part == "":
            ...
            bbox = text_bbox(draw, part, render_font, stroke_width=stroke_width)
            total += bbox[2] - bbox[0]
    return total
...
def _draw_apple_logo(image, xy, render_font, fill):
    ...
    h = size
    w = int(size * 0.82)
    temp = Image.new("RGBA", (w + 10, h + 10), (0, 0, 0, 0))
    d = ImageDraw.Draw(temp)

    # apple body
    left = 4
    top = int(h * 0.20)
    d.ellipse((left, top + int(h*0.12), left + int(w*0.46), top + int(h*0.58)), fill=fill)
    d.ellipse((left + int(w*0.28), top + int(h*0.06), left + int(w*0.76), top + int(h*0.52)), fill=fill)
    d.ellipse((left + int(w*0.10), top + int(h*0.28), left + int(w*0.72), top + int(h*0.82)), fill=fill)
    # bite
    d.ellipse((left + int(w*0.60), top + int(h*0.16), left + int(w*0.92), top + int(h*0.42)), fill=(0, 0, 0, 0))
    # bottom notch
    d.ellipse((left + int(w*0.34), top + int(h*0.62), left + int(w*0.54), top + int(h*0.82)), fill=(0, 0, 0, 0))
    # leaf
    d.ellipse((left + int(w*0.40), 2, left + int(w*0.72), int(h*0.24)), fill=fill)
    temp = temp.rotate(-18, resample=Image.Resampling.BICUBIC, center=(left + int(w*0.56), int(h*0.12)), fillcolor=(0, 0, 0, 0))
    image.paste(temp, (x, y + max(0, int(render_font.requested_size * 0.02))), temp)
    return w + 3
...
def draw_text(image, xy, text, render_font, fill, stroke_width=0, stroke_fill=None):
    ...
    for part in re.split(r'()', text):
        ...
        if part == "":
            ...
            _draw_basic_text(image, (x, y), part, render_font, fill, stroke_width=stroke_width, stroke_fill=stroke_fill)
            x += text_width(draw, part, render_font, stroke_width=stroke_width)
...
def extract_profile_fields(data):
    ...
    nickname = clean_display_text(first_non_empty(basic.get("nickname"), "UNKNOWN"))
    prefix, main_name = split_nickname(nickname)
    if not main_name:
        main_name = nickname

    return {
        "uid": first_non_empty(basic.get("accountId"), "UNKNOWN"),
        "nickname": nickname,
...
def home():
    return jsonify({
        ...
        "endpoint": "/profile-image?uid=PLAYER_UID&key=PANKAJ",
        "example": "/profile-image?uid=8447365517&key=PANKAJ",
        "credits": {
